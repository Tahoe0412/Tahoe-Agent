      ...
      <div className="space-y-6 xl:space-y-5">
            ...
            projectId ? (
              <div className="flex flex-wrap gap-3">
                ...
                <NextStepLink
                  ...
                  label={locale === "en" ? "Next: Prepare Image Jobs" : "下一步：准备图片任务"}
                />
              </div>
            ) : null
          }
        ...
        {recentProjectsUnavailable && !projectId ? (
          <div className="rounded-[24px] border border-[color:color-mix(in_srgb,var(--warning-text)_26%,transparent)] bg-[linear-gradient(135deg,color-mix(in_srgb,var(--warning-bg)_84%,var(--surface-solid)),rgba(255,255,255,0.28))] px-5 py-4 text-sm leading-7 text-[var(--warning-text)] shadow-[0_14px_34px_rgba(145,108,43,0.08)]">
            {locale === "en"
              ? "The project list is temporarily unavailable, but the image-brief page will be ready once workspace data recovers."
              : "当前项目列表暂时不可用，但配图说明页面本身已可访问，等工作区数据恢复后即可继续选择项目。"}
          </div>
        ) : null}

        {loadFailed ? (
          <ErrorPanel
            title={locale === "en" ? "Image Brief Is Temporarily Unavailable" : "配图说明暂时不可用"}
            description={
              locale === "en"
          ...
          <EmptyPanel
            ...
            description={
              locale === "en"
                ? "Open a project first, then continue locking image prompts, references, and asset readiness here."
                : "请先选择项目，再继续确定配图提示词、参考图和素材齐备度。"
            }
            action={<NextStepLink href="/" label={locale === "en" ? "Back to Home" : "先回首页选项目"} />}
          />
        ) : !workspace ? (
          <ErrorPanel
            title={locale === "en" ? "Image Brief Data Unavailable" : "无法读取配图说明数据"}
            description={
              locale === "en"
                ? "The current project could not be found, or this output has not been prepared for image-brief work yet."
                : "当前项目没有找到，或这条内容还没有准备到配图说明这一步。"
            }
            action={<NextStepLink href={`/?projectId=${projectId}`} label={locale === "en" ? "Back to Project" : "返回当前项目"} />}
          />
        ) : workspace.scenePlannerRows.length === 0 ? (
          <EmptyPanel
            title={locale === "en" ? "No Image Brief Yet" : "还没有配图说明初稿"}
            description={
              locale === "en"
                ? "Generate the first image brief directly from the current project, or refine the main draft in Script Lab first."
                : "你可以直接基于当前项目生成第一版配图说明；如果想先细修主稿，也可以先去 Script Lab。"
            }
            action={
              <div className="flex flex-wrap justify-center gap-3">
                <GenerateStoryboardButton projectId={projectId} locale={locale} />
                <NextStepLink href={`/script-lab?projectId=${projectId}`} label={locale === "en" ? "Refine Main Draft" : "先去 Script Lab 细修主稿"} />
              </div>
            }
          />
        ) : (
          <ScenePlannerWorkbench
            ...
            jobs={workspace.latestRenderJobs}
