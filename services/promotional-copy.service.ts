...
function buildEnhancementRules() {
  return [
    "【核心目标】",
    "改写 ≠ 润色。你必须逐段重写正文，而不是保留原稿然后往末尾追加内容。",
    "",
    "【逐段改写要求】",
    "1. 每一段都必须重新组织语言和论证逻辑，不允许直接复制原稿段落。",
    "2. 前2段必须包含品牌/产品身份锚点（品牌名、品类、核心主张），不许延迟到文末。",
    "3. 正文推进结构：场景切入 → 品牌主张 → 差异化解释 → 证明 → 行动引导。",
    "4. 每个证明点必须包含可验证细节（数据、地名、材料名、第三方背书），不能只有形容词。",
    "5. CTA 必须承接正文最后一个论证点的关键词，不能是泛用的'了解更多'。",
    "6. 标题至少一条含数据或对比钩子。",
    "",
    "【禁止行为】",
    "- 禁止保留原稿的任何整段，必须逐段重写。",
    "- 禁止在原稿末尾追加新段落来'凑改写'，必须对每一段落做实质修改。",
    "- 禁止只换几个形容词当作改写。",
    "- 禁止输出策划建议（如'建议围绕...'）。",
  ].join("\n");
}
  ...
  async diagnoseAndEnhance(
    ...
    if (canUseModelRoute("PROMOTIONAL_COPY", settings)) {
      ...
      const rewriteResult = await generateStructuredJson<Record<string, unknown>>({
        ...
        systemPrompt: [
          "你是品牌文案总监。你的任务是根据质量诊断报告，对原稿做逐段重写。",
          "核心要求：每一段都必须重新组织，禁止保留原稿整段不动然后在末尾追加内容。",
          "你要解决诊断中的每一个 [问题]，执行每一个 [动作]，同时保护列出的优点。",
          "输出合法 JSON，只输出改写后的文案字段。",
        ].join(" "),
        userPrompt: [
          "=== 需要改写的原稿 ===",
          `角度：${currentDraft.master_angle}`,
          ...
          context?.styleReferenceInsight
            ...
            : "",
          "",
          truncatedBrief ? `项目上下文：\n${truncatedBrief}` : "",
          "",
          "输出要求：master_angle、headline_options(>=3)、hero_copy(前2句含品牌身份)、long_form_copy(逐段重写,不要保留原稿整段)、proof_points(>=3,带数据)、call_to_action(承接正文)。",
        ].filter(Boolean).join("\n"),
      });
