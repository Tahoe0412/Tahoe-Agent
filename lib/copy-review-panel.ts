...
export function getChineseMediaCalibrationNotes(params?: {
  ...
  const baseNotes = [
    ...
    "避免 AI 味：不要用对称清单、万能转接词、空泛宏大词和每段都很正确但没有作者脾气的表达。",
    "复杂技术要有人话翻译：用现实工作、产品体验、成本感受或具体操作解释，不要只堆模型名和参数。",
  ...
  if (params?.target === "MASTER_COPY") {
    baseNotes.splice(
      ...
      "好的科技评论通常会先建立一个能贯穿全文的核心隐喻或冲突，再把事实逐层挂上去；弱稿往往只是把材料按顺序摊开。",
      "第一屏如果像通稿、百科或模型卡摘要，即使信息正确也要降分。",
    );
  }
...
export function buildAudiencePanelPrompt(params: {
  ...
  return {
    systemPrompt: [
      ...
      "你要特别识别“AI味”：机械三段式、过度对称、开头像通稿、连续使用万能转接词、只有正确摘要没有真人判断、只有抽象词没有生活场景。",
      "参考高质量中文科技媒体/创作者的共同方法，而不是模仿具体人：事实要像媒体一样扎实，解释要像作者一样有人话，判断要能被普通读者转述。",
