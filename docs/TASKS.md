...
- **Current slice (Done)**:
  ...
  - local Qwen calls now pin conservative production parameters from environment variables: `QWEN_TEMPERATURE=0.35`, `QWEN_TOP_P=0.85`, `QWEN_MAX_TOKENS=8192`; LM Studio's current loaded context window is documented as `QWEN_CONTEXT_WINDOW=131072`.
  ...
  - the frontend anti-card rewrite has now been expanded system-wide across the main workbenches and supporting pages. Large rounded cards, glow/gradient treatments, dark selected panels, hard-coded colored alerts, white translucent UI, circular score/chip patterns, and hover-lift shadows were replaced with tokenized flat sections, fine dividers, compact rows, square/low-radius controls, and calmer status treatments. Backend services, route contracts, Prisma schema, and generation logic were not changed.
  - Daily Run and Script Lab have now been simplified for low-barrier daily publishing. Daily Run leads with `今日选题`, the search box, three account topic picks, one `生成文章包` action, and folded source/reason/alternate-topic details. Script Lab now reads as `成稿编辑`, keeps正文检查/素材来源/复核评分 collapsed by default, and moves image-brief internals behind `高级：配图细节`.
  - the frontend naming and palette have now been tuned again for a more editorial, less Claude-like first impression: the main production labels are `今日选题`, `成稿编辑`, `配图 brief`, and `出图台`, and the global tokens now use a cool porcelain / ink-green / teal palette with cobalt, plum, and coral accents instead of warm brown and terracotta dominance.
  - project-root `DESIGN.md` now defines Tahoe's UI taste for future agents: WIRED-style editorial density and broadsheet hierarchy as the structural reference, Tahoe's own cool ink-green / teal token system as the product palette, and explicit anti-patterns against Claude-like warm terracotta, generic AI dashboards, and terminal-only styling.
  - the shared frontend shell and the daily publishing path now follow that `DESIGN.md` baseline more tightly: default theme starts in light editorial mode, app/header/panel/state primitives use smaller radii and fine-divider structure, Daily Run stays search-first, and Script Lab keeps正文/标题/发布文案 in the main path while moving配图/复核 details behind folds. Article-line next actions now jump to发布文案 instead of routing users into Marketing Ops.
  - the next visible frontend sweep flattened the remaining project-context and script-preview cards in Script Lab, including the compact project header, script header,正文检查, source list, tag input, error notice, and mobile sidebar close control.
  - local LM Studio Qwen generation was exercised for all three Daily Run accounts on 2026-04-30. Tahoe now sends `reasoning_effort: "none"` for local Qwen chat-completions, preventing LM Studio from returning empty `content` with all text trapped in `reasoning_content`.
  - the 2026-04-30 Qwen Daily Run created three local Script Lab projects: AI快讯 `cmol95fyt0008s0j9ixzy9cxf`, 全球股市 `cmol9777a000rs0j9wtd4w52p`, and 消费时尚 `cmol98iss000ws0j9tfjdjk2y`.
  - the 2026-05-12 Qwen Daily Run created another three local Script Lab projects through platform APIs only: AI快讯 `cmp27ijyt0000s0rbdgpuvt61`, 全球股市 `cmp27kqdy0007s0rbedoumpr3`, and 消费时尚 `cmp27mgdl000es0rbmjksdfmu`. This run completed title pack + publish copy and skipped automatic storyboard/image generation to keep the daily article-output lane isolated.
- **Next**:
  - formalize "每日三篇文章输出" as a dedicated Daily Run board with its own queue/status/readiness model, so this recurring work is coordinated inside Daily Run instead of bleeding into Marketing Ops, Scene Planner, Render Lab, or broader project-management surfaces
  ...
  - evaluate whether article-writing routes should later get a slightly higher local-Qwen temperature than review/JSON routes, after parse reliability is measured
  - reduce background packaging context for local Qwen, because audience review, image-brief generation, and some title-pack calls can still exceed the LM Studio context window after a long article draft is created
